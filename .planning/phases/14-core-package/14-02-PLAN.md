...
Update vite.config.ts to build multiple entry points:
```typescript
...
Note: The vite-config library function may need to be updated to handle object entry. Check if it works; if not, use inline defineConfig instead.
...
```
  ...
  <verify>Run `pnpm --filter @lit-ui/core build` and verify dist/tokens/index.js exists</verify>
  <done>Package exports /tokens and /utils subpaths, build includes all entry points</done>
</task>
