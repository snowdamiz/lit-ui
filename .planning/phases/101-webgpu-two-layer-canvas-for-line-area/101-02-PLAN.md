...
private _syncCoordinates(): void {
  ...
  // WEBGPU-02 coordinate sync: ARCHITECTURAL NOTE — WEBGPU-02 specifies convertToPixel() for
  // coordinate sync, but ChartGPU.setZoomRange() operates in percent-space [0, 100], not pixel-space.
  // Using getOption().dataZoom[0].start/end (percent-space) is therefore the architecturally correct
  // approach: it maps directly to what ChartGPU needs without an additional pixel→percent conversion.
  //
  // If SC2 manual verification (no pixel offset after DataZoom) fails, the fallback path is:
  //   const [pixelStart] = this._chart.convertToPixel({ xAxisIndex: 0 }, [0, 0]);
  //   const [pixelEnd]   = this._chart.convertToPixel({ xAxisIndex: 0 }, [100, 0]);
  // Then derive percent from pixel extents and the container width. This is more work and should
  // only be pursued if the percent-space sync shows visible misalignment during SC2 verification.
